...
// Генерация случайного ключа (32 байта для AES-256)
...
!(function (e) {
    function t() {
        ...
        const c = qrcodegen.QrSegment.makeSegments(r);//создание сегментов
        const i = parseInt(document.getElementById("force-min-version").value, 10);//версия QR-кода
        const l = parseInt(document.getElementById("force-mask-pattern").value, 10);//маска QR-кода
        const size = document.getElementById("force-size").value;//размер QR-кода
        const u = qrcodegen.QrCode.encodeSegments(c, t, i, 40, l, 0);//кодирование
        const a = document.getElementById("light-color-input").value;//цвет фона
        const d = document.getElementById("dark-color-input").value;//цвет модулей
        const da = document.getElementById("s_light-color-input").value;//2 цвет, если включен градиент 

        {
            ...
            (function (e, t, n, o, r, da, c) {
                ...
                if (removeBackground && imageInput.files.length > 0) {
                    const img = new Image();
                    img.src = "res/picture/backgroung.png";
                    img.onload = function () {
                        l.drawImage(img, x, y, imgSize, imgSize); // Рисуем фоновое изображение, поверх модулей
                        ...
                        if (imageInput.files.length > 0) {
                            ...
                            img1.onload = function () {
                                l.drawImage(img1, x, y, imgSize, imgSize);
                            };
                        }
                    };
                } else {
    ...
    ((app1 || (app1 = {})).handleVersionMinMax = function (e) {
        ...
        (function () {//При изменение значений элементов range - меняются и значения input
