...
!(function (e) {
    function t() {
        ...
        const t = o("errcorlvl-medium").checked ? qrcodegen.QrCode.Ecc.MEDIUM : 
                  o("errcorlvl-quartile").checked ? qrcodegen.QrCode.Ecc.QUARTILE : 
                  o("errcorlvl-high").checked ? qrcodegen.QrCode.Ecc.HIGH : 
                ...
                for (let c = -n; c < e.size + n; c++) {
                    for (let j = -n; j < e.size + n; j++) {
                        ...
                        if (removeBackground && 
                            j >= x/t && j < (x + imgSize)/t &&
                        c >= y/t && c < (y + imgSize)/t) {
                            continue; // Пропускаем модули в области изображения
                        }
